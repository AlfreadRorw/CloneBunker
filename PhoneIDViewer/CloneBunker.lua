...
-- ================= ICON BUILDERS (FIXED - SIZE & SETTINGS GEAR) =================
local iconBuilders = {
    ...
    Reset = function(p, c)
        ...
        innerCircle.Position = UDim2.new(0.5, -7, 0.5, -7)
        innerCircle.Position = UDim2.new(0.5, -7, 0.36, 0)
        innerCircle.BackgroundColor3 = Color3.new(1, 1, 1)
        corner(innerCircle, 100)
        
        -- Arrow head top-right
        local arrow1 = Instance.new("Frame", p)
        arrow1.Size = UDim2.new(0, 6, 0, 6)
        arrow1.Position = UDim2.new(0.5, 10, 0.24, 0)
        arrow1.BackgroundColor3 = c
        arrow1.Rotation = 45
        corner(arrow1, 1)
        
        local shaft1 = Instance.new("Frame", p)
        shaft1.Size = UDim2.new(0, 8, 0, 2)
        shaft1.Position = UDim2.new(0.5, 6, 0.28, 0)
        shaft1.BackgroundColor3 = c
        corner(shaft1, 1)
        
        -- Arrow head bottom-left
        local arrow2 = Instance.new("Frame", p)
        arrow2.Size = UDim2.new(0, 6, 0, 6)
        arrow2.Position = UDim2.new(0.5, -15, 0.52, 0)
        arrow2.BackgroundColor3 = c
        arrow2.Rotation = 45
        corner(arrow2, 1)
        
        local shaft2 = Instance.new("Frame", p)
        shaft2.Size = UDim2.new(0, 8, 0, 2)
        shaft2.Position = UDim2.new(0.5, -13, 0.56, 0)
        shaft2.BackgroundColor3 = c
        corner(shaft2, 1)
    end,
...
-- ================= RESET APP (FIXED - USING OUTFITS + CURRENT AVATAR) =================
local function openResetApp()
    -- ==================== RESET BUTTON ====================
    local resetCard = Instance.new("Frame", appContent)
    resetCard.Size = UDim2.new(1, 0, 0, 50)
    resetCard.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    resetCard.LayoutOrder = 0
    corner(resetCard, 14)
    stroke(resetCard, Color3.fromRGB(225, 225, 230), 1, 0.3)
    
    local resetTitle = Instance.new("TextLabel", resetCard)
    resetTitle.Size = UDim2.new(1, -24, 0, 22)
    resetTitle.Position = UDim2.new(0, 12, 0, 6)
    resetTitle.BackgroundTransparency = 1
    resetTitle.Text = "Reset Character"
    resetTitle.TextColor3 = T.Text
    ...
    resetBtn.Size = UDim2.new(1, -24, 0, 22)
    resetBtn.Position = UDim2.new(0, 12, 0, 24)
    resetBtn.BackgroundColor3 = T.Red
    resetBtn.Text = "RESET (re)"
    ...
    resetBtn.MouseButton1Click:Connect(function()
        resetCharacter()
        showDynamicNotification("Character reset!", T.Green)
    end)
    
    -- ==================== MY OUTFITS SECTION ====================
    local outfitHeader = Instance.new("Frame", appContent)
    outfitHeader.Size = UDim2.new(1, 0, 0, 32)
    outfitHeader.BackgroundTransparency = 1
    outfitHeader.LayoutOrder = 1
    
    local outfitAccent = Instance.new("Frame", outfitHeader)
    outfitAccent.Size = UDim2.new(0, 3, 0, 20)
    outfitAccent.Position = UDim2.new(0, 0, 0.5, -10)
    outfitAccent.BackgroundColor3 = T.Accent
    corner(outfitAccent, 2)
    
    local outfitTitle = Instance.new("TextLabel", outfitHeader)
    outfitTitle.Size = UDim2.new(1, -16, 0, 18)
    outfitTitle.Position = UDim2.new(0, 8, 0, 2)
    outfitTitle.BackgroundTransparency = 1
    outfitTitle.Text = "My Outfits & Current Avatar"
    outfitTitle.TextColor3 = T.Text
    outfitTitle.Font = Enum.Font.GothamBlack
    outfitTitle.TextSize = 13
    outfitTitle.TextXAlignment = Enum.TextXAlignment.Left
    
    local outfitSub = Instance.new("TextLabel", outfitHeader)
    outfitSub.Size = UDim2.new(1, -16, 0, 12)
    outfitSub.Position = UDim2.new(0, 8, 0, 20)
    outfitSub.BackgroundTransparency = 1
    outfitSub.Text = "Switch outfit & reset, or save for respawn"
    outfitSub.TextColor3 = T.Text2
    outfitSub.Font = Enum.Font.Gotham
    outfitSub.TextSize = 8
    outfitSub.TextXAlignment = Enum.TextXAlignment.Left
    ...
    loadingCard.Size = UDim2.new(1, 0, 0, 35)
    ...
    -- Load saved outfit
    local savedOutfit = loadJSON("PhoneIDViewer_SavedOutfit.json") or {}
    ...
    -- Fetch outfits (PASTI BERHASIL karena API outfits lebih stabil)
    coroutine.wrap(function()
        local userId = LocalPlayer.UserId
        local outfitList = {}
        
        -- ===== METHOD 1: GET OUTFITS =====
        ...
        -- ===== METHOD 2: CURRENTLY WORN (PASTI BERHASIL) =====
        -- Tambahkan current avatar di awal list
        local currentRaw = httpGet("https://avatar.roblox.com/v1/users/" .. userId .. "/currently-wearing")
        local currentIds = {}
        
        if currentRaw then
            local ok, data = pcall(function() return HttpService:JSONDecode(currentRaw) end)
            if ok and data and data.assetIds and #data.assetIds > 0 then
                for _, assetId in ipairs(data.assetIds) do
                    table.insert(currentIds, tostring(assetId))
                end
            end
        end
        
        -- Jika currentIds kosong, ambil dari character langsung
        if #currentIds == 0 then
            local charItems = getItems(LocalPlayer)
            for _, it in ipairs(charItems) do
                table.insert(currentIds, it.Value)
            end
        end
        
        -- Selalu tambahkan Current Avatar di paling atas
        if #currentIds > 0 then
            table.insert(outfitList, 1, {
                id = "current",
                name = "Current Avatar (Now)",
                assets = currentIds
            })
        end
        ...
        if #outfitList == 0 then
            ...
            emptyMsg.Position = UDim2.new(0, 10, 0, 70)
            emptyMsg.BackgroundTransparency = 1
            emptyMsg.Text = "Create outfits in Avatar Editor first!\n(Garis 3 > Outfits > Create Outfit)"
        ...
        -- ===== SAVED OUTFIT BANNER =====
        if savedOutfit.name then
            local savedBanner = Instance.new("Frame", contentFrame)
            savedBanner.Size = UDim2.new(1, 0, 0, 28)
            savedBanner.BackgroundColor3 = Color3.fromRGB(235, 255, 235)
            corner(savedBanner, 8)
            stroke(savedBanner, T.Green, 1.5, 0.2)
            
            local savedText = Instance.new("TextLabel", savedBanner)
            savedText.Size = UDim2.new(1, -46, 1, 0)
            savedText.Position = UDim2.new(0, 10, 0, 0)
            savedText.BackgroundTransparency = 1
            savedText.Text = "Saved: " .. savedOutfit.name .. " (auto-wear)"
            savedText.TextColor3 = T.Green
            savedText.Font = Enum.Font.GothamBold
            savedText.TextSize = 9
            savedText.TextXAlignment = Enum.TextXAlignment.Left
            
            local clearBtn = Instance.new("TextButton", savedBanner)
            clearBtn.Size = UDim2.new(0, 36, 0, 18)
            clearBtn.Position = UDim2.new(1, -40, 0.5, -9)
            ...
            clearBtn.MouseButton1Click:Connect(function()
                saveJSON("PhoneIDViewer_SavedOutfit.json", {})
                showDynamicNotification("Saved outfit cleared!", T.Green)
                refreshCurr()
            end)
        end
        ...
        grid.CellSize = UDim2.new(0.5, -5, 0, 175)
        ...
        for i, outfit in ipairs(outfitList) do
            ...
            local isSaved = (tostring(outfit.id) == tostring(savedOutfit.id))
            local isCurrent = (outfit.id == "current")
            
            if isSaved then
                stroke(card, T.Green, 2.5, 0)
            ...
            -- Badge "CURRENT" untuk current avatar
            if isCurrent then
                local currentBadge = Instance.new("Frame", card)
                currentBadge.Size = UDim2.new(0, 50, 0, 13)
                currentBadge.Position = UDim2.new(0, 4, 0, 4)
                currentBadge.BackgroundColor3 = Color3.fromRGB(100, 150, 255)
                currentBadge.BackgroundTransparency = 0.8
                corner(currentBadge, 6)
                
                local badgeText = Instance.new("TextLabel", currentBadge)
                badgeText.Size = UDim2.new(1, 0, 1, 0)
                badgeText.BackgroundTransparency = 1
                badgeText.Text = "CURRENT"
                badgeText.TextColor3 = Color3.fromRGB(100, 150, 255)
            ...
            nameLbl.Position = UDim2.new(0, 6, 0, 86)
            nameLbl.BackgroundTransparency = 1
            ...
            if isSaved then
                ...
                badge.Position = UDim2.new(0.5, -22, 0, 112)
                ...
                badgeText.Size = UDim2.new(1, 0, 1, 0)
                badgeText.BackgroundTransparency = 1
                badgeText.Text = "Saved"
                badgeText.TextColor3 = T.Green
                badgeText.Font = Enum.Font.GothamBold
            ...
            btnContainer.Size = UDim2.new(1, -12, 0, 44)
            btnContainer.Position = UDim2.new(0, 6, 0, 125)
            btnContainer.BackgroundTransparency = 1
            
            -- Switch & Reset button
            local switchBtn = Instance.new("TextButton", btnContainer)
            switchBtn.Size = UDim2.new(1, 0, 0, 24)
            switchBtn.BackgroundColor3 = T.Accent
            switchBtn.Text = "Wear & Reset"
            switchBtn.TextColor3 = T.OnAccent
            ...
            switchBtn.MouseButton1Click:Connect(function()
                switchBtn.Text = "..."
                switchBtn.BackgroundColor3 = T.Gold
                
                local function applyOutfit(ids)
                    if ids and #ids > 0 then
                        ...
                        resetCharacter()
                        switchBtn.Text = "Done!"
                        switchBtn.BackgroundColor3 = T.Green
                        showDynamicNotification("Outfit applied: " .. outfit.name, T.Green)
                        task.wait(1.5)
                        switchBtn.Text = "Wear & Reset"
                        switchBtn.BackgroundColor3 = T.Accent
                    else
                        switchBtn.Text = "Empty!"
                        switchBtn.BackgroundColor3 = T.Red
                        showDynamicNotification("Outfit has no items!", T.Red)
                        task.wait(1.5)
                        switchBtn.Text = "Wear & Reset"
                        switchBtn.BackgroundColor3 = T.Accent
                    end
                ...
                if outfit.assets and #outfit.assets > 0 then
                    applyOutfit(outfit.assets)
                elseif isCurrent then
                    -- Current: ambil dari character langsung
                    local charItems = getItems(LocalPlayer)
                    local ids = {}
                    for _, it in ipairs(charItems) do
                        table.insert(ids, it.Value)
            ...
            -- Save for Respawn button
            local saveBtn = Instance.new("TextButton", btnContainer)
            saveBtn.Size = UDim2.new(1, 0, 0, 18)
            saveBtn.Position = UDim2.new(0, 0, 0, 26)
            saveBtn.BackgroundColor3 = isSaved and T.Green or Color3.fromRGB(240, 248, 240)
            saveBtn.Text = isSaved and "Saved for Respawn" or "Save for Respawn"
            saveBtn.TextColor3 = isSaved and T.OnAccent or Color3.fromRGB(0, 100, 0)
            saveBtn.Font = Enum.Font.GothamBold
            saveBtn.TextSize = 7
            saveBtn.AutoButtonColor = false
            corner(saveBtn, 5)
            stroke(saveBtn, T.Green, 1, 0.3)
            pressFX(saveBtn)
            
            saveBtn.MouseButton1Click:Connect(function()
                -- Fetch assets jika belum ada
                if not outfit.assets or #outfit.assets == 0 then
                    if isCurrent then
                        local charItems = getItems(LocalPlayer)
                        outfit.assets = {}
                        for _, it in ipairs(charItems) do
                            table.insert(outfit.assets, it.Value)
                        end
                    else
                        local detailRaw = httpGet("https://avatar.roblox.com/v1/outfits/" .. outfit.id .. "/details")
                        if detailRaw then
                            ...
                            if ok and detail and detail.assets then
                                ...
                                for _, asset in ipairs(detail.assets) do
                                    if asset.id and type(asset.id) == "number" then
                                        table.insert(outfit.assets, tostring(asset.id))
                                    end
                                end
                            end
                        end
                ...
                local saveData = {
                    id = outfit.id,
                    name = outfit.name,
                    assets = outfit.assets or {}
                }
                saveJSON("PhoneIDViewer_SavedOutfit.json", saveData)
                showDynamicNotification("Saved: " .. outfit.name, T.Green)
                refreshCurr()
            end)
        end
        ...
        refreshBtn.BackgroundColor3 = Color3.fromRGB(240, 240, 245)
        ...
        corner(refreshBtn, 8)
        stroke(refreshBtn, T.Border, 1, 0.3)
        pressFX(refreshBtn)
...
-- ================= AUTO-WEAR ON RESPAWN =================
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(1)
    local savedOutfit = loadJSON("PhoneIDViewer_SavedOutfit.json") or {}
    if savedOutfit.assets and #savedOutfit.assets > 0 then
        task.wait(0.5)
        fireHat(savedOutfit.assets)
        showDynamicNotification("Auto-wearing: " .. (savedOutfit.name or "Outfit"), T.Green)
    end
end)
...
buildAppIcon("Reset",2,dockBg,function() openApp("Reset",openResetApp) end)
