...
Message = function(p, c)
    ...
    bubble.Size = UDim2.new(0, 28, 0, 22)
    bubble.Position = UDim2.new(0.5, -14, 0.26, 0)
    bubble.BackgroundColor3 = c
    corner(bubble, 8)
    
    -- Tail
    local tail = Instance.new("Frame", p)
    tail.Size = UDim2.new(0, 7, 0, 7)
    ...
    for i = 1, 3 do
        ...
        dot.Size = UDim2.new(0, 4, 0, 4)
        dot.Position = UDim2.new(0, 4 + (i-1)*8, 0.5, -2)
...
-- ================= MESSAGE SYSTEM (FULL) =================
-- Taruh di bagian APPS, sebelum buildAppIcon

-- Firebase helpers (gunakan yang sudah ada, atau pakai JSONBin)
-- Pastikan fungsi firebaseGet, firebaseSet, firebaseDelete sudah ada di HELPERS

local activeMessageNotif = nil
local MESSAGE_CHECK_INTERVAL = 6
local openedConversation = nil

-- ================= SEND MESSAGE =================
local function sendMessage(toUserId, text)
...
local function markAllRead(fromUserId)
    local msgs = firebaseGet("/messages/" .. tostring(LocalPlayer.UserId))
    if not msgs or type(msgs) ~= "table" then return end
    for msgId, msg in pairs(msgs) do
        if tostring(msg.fromId) == tostring(fromUserId) and not msg.read then
            firebaseSet("/messages/" .. tostring(LocalPlayer.UserId) .. "/" .. msgId .. "/read", true)
        end
    end
end
...
local function getUnreadCount()
    local msgs = firebaseGet("/messages/" .. tostring(LocalPlayer.UserId))
    local count = 0
    if not msgs or type(msgs) ~= "table" then return 0 end
...
-- ================= NOTIF POPUP (IPHONE STYLE) =================
local function showMessageNotif(fromDisplay, fromId, text, onReply)
    ...
    card.Position = UDim2.new(0.5, -150, 0, -90)
    card.BackgroundColor3 = Color3.fromRGB(28, 28, 32)
    card.BackgroundTransparency = 0.08
    card.BorderSizePixel = 0
    card.ZIndex = 9998
    corner(card, 20)
    stroke(card, Color3.fromRGB(60, 60, 65), 1, 0)

    local appIconFrame = Instance.new("Frame", card)
    appIconFrame.Size = UDim2.new(0, 36, 0, 36)
    appIconFrame.Position = UDim2.new(0, 12, 0.5, -18)
    appIconFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    appIconFrame.ZIndex = 9999
    corner(appIconFrame, 10)
    stroke(appIconFrame, Color3.fromRGB(60, 60, 65), 1, 0)

    local avatarImg = Instance.new("ImageLabel", appIconFrame)
    avatarImg.Size = UDim2.new(1, -2, 1, -2)
    avatarImg.Position = UDim2.new(0.5, 0, 0.5, 0)
    avatarImg.AnchorPoint = Vector2.new(0.5, 0.5)
    avatarImg.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
    avatarImg.Image = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. tostring(fromId) .. "&width=100&height=100&format=png"
    avatarImg.ZIndex = 10000
    corner(avatarImg, 9)

    local onlineDotNotif = Instance.new("Frame", appIconFrame)
    onlineDotNotif.Size = UDim2.new(0, 9, 0, 9)
    onlineDotNotif.Position = UDim2.new(1, -4, 1, -4)
    onlineDotNotif.BackgroundColor3 = Color3.fromRGB(0, 220, 100)
    onlineDotNotif.ZIndex = 10001
    corner(onlineDotNotif, 100)
    stroke(onlineDotNotif, Color3.fromRGB(28, 28, 32), 2, 0)

    local appLabel = Instance.new("TextLabel", card)
    appLabel.Size = UDim2.new(1, -70, 0, 12)
    appLabel.Position = UDim2.new(0, 56, 0, 10)
    appLabel.BackgroundTransparency = 1
    appLabel.Text = "MESSAGES  *  SEKARANG"
    appLabel.TextColor3 = Color3.fromRGB(160, 160, 165)
    appLabel.Font = Enum.Font.GothamBold
    appLabel.TextSize = 7
    appLabel.TextXAlignment = Enum.TextXAlignment.Left
    appLabel.ZIndex = 9999

    local senderLbl = Instance.new("TextLabel", card)
    senderLbl.Size = UDim2.new(1, -70, 0, 16)
    senderLbl.Position = UDim2.new(0, 56, 0, 22)
    senderLbl.BackgroundTransparency = 1
    senderLbl.Text = fromDisplay
    senderLbl.TextColor3 = Color3.new(1, 1, 1)
    senderLbl.Font = Enum.Font.GothamBlack
    senderLbl.TextSize = 13
    senderLbl.TextXAlignment = Enum.TextXAlignment.Left
    senderLbl.ZIndex = 9999

    local msgPreview = Instance.new("TextLabel", card)
    msgPreview.Size = UDim2.new(1, -70, 0, 14)
    msgPreview.Position = UDim2.new(0, 56, 0, 40)
    msgPreview.BackgroundTransparency = 1
    msgPreview.Text = #text > 38 and text:sub(1, 38) .. "..." or text
    msgPreview.TextColor3 = Color3.fromRGB(180, 180, 185)
    msgPreview.Font = Enum.Font.Gotham
    msgPreview.TextSize = 11
    msgPreview.TextXAlignment = Enum.TextXAlignment.Left
    msgPreview.ZIndex = 9999

    local dismissHint = Instance.new("Frame", card)
    dismissHint.Size = UDim2.new(0, 32, 0, 3)
    dismissHint.Position = UDim2.new(0.5, -16, 1, -8)
    dismissHint.BackgroundColor3 = Color3.fromRGB(80, 80, 85)
    dismissHint.BorderSizePixel = 0
    dismissHint.ZIndex = 9999
    corner(dismissHint, 2)

    tween(card, {Position = UDim2.new(0.5, -150, 0, 12)}, 0.4, Enum.EasingStyle.Back)

    local dismissed = false
    local replyExpanded = false

    task.spawn(function()
        task.wait(5)
        if not dismissed and not replyExpanded then
            dismissed = true
            tween(card, {Position = UDim2.new(0.5, -150, 0, -90)}, 0.3, Enum.EasingStyle.Quart)
            task.wait(0.35)
            pcall(function() notifGui:Destroy() end)
            activeMessageNotif = nil
        end
    ...
    clickBtn.Text = ""
    clickBtn.ZIndex = 10002
    clickBtn.AutoButtonColor = false

    clickBtn.MouseButton1Click:Connect(function()
        if replyExpanded then return end
        replyExpanded = true
        dismissed = false

        tween(card, {Size = UDim2.new(0, 300, 0, 148)}, 0.3, Enum.EasingStyle.Back)

        local replyBg = Instance.new("Frame", card)
        replyBg.Size = UDim2.new(1, -16, 0, 44)
        replyBg.Position = UDim2.new(0, 8, 0, 86)
        replyBg.BackgroundColor3 = Color3.fromRGB(45, 45, 50)
        replyBg.BackgroundTransparency = 0
        replyBg.BorderSizePixel = 0
        replyBg.ZIndex = 10003
        corner(replyBg, 12)
        stroke(replyBg, Color3.fromRGB(70, 70, 76), 1, 0)

        local replyInput = Instance.new("TextBox", replyBg)
        replyInput.Size = UDim2.new(1, -52, 1, -10)
        replyInput.Position = UDim2.new(0, 10, 0, 5)
        replyInput.BackgroundTransparency = 1
        replyInput.PlaceholderText = "Balas pesan..."
        replyInput.PlaceholderColor3 = Color3.fromRGB(110, 110, 115)
        replyInput.Text = ""
        ...
        local actionRow = Instance.new("Frame", card)
        actionRow.Size = UDim2.new(1, -16, 0, 28)
        actionRow.Position = UDim2.new(0, 8, 0, 114)
        actionRow.BackgroundTransparency = 1
        actionRow.ZIndex = 10003

        local closeBtn2 = Instance.new("TextButton", actionRow)
        closeBtn2.Size = UDim2.new(0.48, 0, 1, 0)
        closeBtn2.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
        closeBtn2.Text = "Tutup"
        closeBtn2.TextColor3 = Color3.fromRGB(180, 180, 185)
        closeBtn2.Font = Enum.Font.GothamBold
        closeBtn2.TextSize = 10
        closeBtn2.AutoButtonColor = false
        closeBtn2.ZIndex = 10004
        corner(closeBtn2, 8)
        closeBtn2.MouseButton1Click:Connect(function()
            dismissed = true
            tween(card, {Position = UDim2.new(0.5, -150, 0, -90)}, 0.3)
            task.wait(0.35)
            pcall(function() notifGui:Destroy() end)
            activeMessageNotif = nil
        end)

        local openAppBtn = Instance.new("TextButton", actionRow)
        openAppBtn.Size = UDim2.new(0.48, 0, 1, 0)
        openAppBtn.Position = UDim2.new(0.52, 0, 0, 0)
        openAppBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        openAppBtn.Text = "Buka App"
        openAppBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
        openAppBtn.Font = Enum.Font.GothamBold
        openAppBtn.TextSize = 10
        openAppBtn.AutoButtonColor = false
        openAppBtn.ZIndex = 10004
        corner(openAppBtn, 8)
        openAppBtn.MouseButton1Click:Connect(function()
            dismissed = true
            tween(card, {Position = UDim2.new(0.5, -150, 0, -90)}, 0.3)
            task.wait(0.35)
            pcall(function() notifGui:Destroy() end)
        ...
        local function doSend()
            ...
            if onReply then pcall(onReply, txt) end
            task.wait(0.5)
            dismissed = true
            tween(card, {Position = UDim2.new(0.5, -150, 0, -90)}, 0.3)
            task.wait(0.35)
            pcall(function() notifGui:Destroy() end)
            activeMessageNotif = nil
            showDynamicNotification("Pesan terkirim ke " .. fromDisplay, Color3.fromRGB(255, 255, 255))
        end
...
-- ================= MESSAGE NOTIF CHECKER =================
local function checkMessageNotif()
    while true do
        ...
        if notif and type(notif) == "table" and notif.from then
            if tostring(notif.fromId) ~= tostring(LocalPlayer.UserId) then
                if not (appTitle.Text == "Messages" and openedConversation == notif.fromId) then
                    firebaseDelete("/message_notifs/" .. tostring(LocalPlayer.UserId))
                    showMessageNotif(
                        notif.fromDisplay or notif.from,
                        notif.fromId,
                        notif.text,
                        nil
                    )
...
-- ================= MESSAGE APP =================
local function openMessageApp()
    local onlineData = firebaseGet("/online_players") or {}
    local myMessages = firebaseGet("/messages/" .. tostring(LocalPlayer.UserId)) or {}

    local unreadBySender = {}
    local lastMsgBySender = {}
    for _, msg in pairs(myMessages) do
        local sid = tostring(msg.fromId)
        if not unreadBySender[sid] then unreadBySender[sid] = 0 end
        if not msg.read then unreadBySender[sid] = unreadBySender[sid] + 1 end
        if not lastMsgBySender[sid] or msg.timestamp > lastMsgBySender[sid].timestamp then
            lastMsgBySender[sid] = msg
        end
    end
    ...
    headerSub.BackgroundTransparency = 1
    headerSub.Text = "Pesan antar member"
    headerSub.TextColor3 = Color3.fromRGB(120, 120, 125)
    headerSub.Font = Enum.Font.Gotham
    headerSub.TextSize = 8
    headerSub.TextXAlignment = Enum.TextXAlignment.Left
    ...
    for _, msg in pairs(myMessages) do
        local sid = tostring(msg.fromId)
        if not addedIds[sid] and sid ~= tostring(LocalPlayer.UserId) then
            addedIds[sid] = true
            table.insert(contacts, {
                userId = msg.fromId,
                displayName = msg.fromDisplay or msg.from,
                username = msg.from,
                unread = unreadBySender[sid] or 0,
                lastMsg = lastMsgBySender[sid]
            })
        end
    end

    for _, p in pairs(onlineData) do
        local sid = tostring(p.userId)
        if not addedIds[sid] and sid ~= tostring(LocalPlayer.UserId) then
            addedIds[sid] = true
            table.insert(contacts, {
                userId = p.userId,
                displayName = p.displayName or p.username,
                username = p.username,
                unread = 0,
                isOnline = true
            })
    ...
    table.sort(contacts, function(a, b)
        if a.unread ~= b.unread then return a.unread > b.unread end
        if a.isOnline ~= b.isOnline then return a.isOnline end
        return (a.lastMsg and a.lastMsg.timestamp or 0) > (b.lastMsg and b.lastMsg.timestamp or 0)
    end)

    if #contacts == 0 then
        local emptyCard = Instance.new("Frame", contentFrame)
        emptyCard.Size = UDim2.new(1, 0, 0, 80)
        emptyCard.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
        corner(emptyCard, 14)
        
        local emptyText = Instance.new("TextLabel", emptyCard)
        emptyText.Size = UDim2.new(1, 0, 1, 0)
        emptyText.BackgroundTransparency = 1
        emptyText.Text = "Belum ada percakapan"
        emptyText.TextColor3 = Color3.fromRGB(100, 100, 105)
        emptyText.Font = Enum.Font.GothamBold
        emptyText.TextSize = 12
        return
    end
    ...
    for i, contact in ipairs(contacts) do
        local hasUnread = contact.unread > 0

        local row = Instance.new("TextButton", contentFrame)
        row.Size = UDim2.new(1, 0, 0, 62)
        row.BackgroundColor3 = hasUnread and Color3.fromRGB(22, 22, 28) or Color3.fromRGB(16, 16, 20)
        ...
        if hasUnread then
            stroke(row, Color3.fromRGB(255, 255, 255), 1, 0.7)
        end
        ...
        avatar.Image = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. tostring(contact.userId) .. "&width=100&height=100&format=png"
        corner(avatar, 100)

        if contact.isOnline then
            local odot = Instance.new("Frame", row)
            odot.Size = UDim2.new(0, 8, 0, 8)
            odot.Position = UDim2.new(0, 38, 0.5, 10)
            odot.BackgroundColor3 = Color3.fromRGB(0, 220, 100)
            corner(odot, 100)
        end
        ...
        nameLbl.BackgroundTransparency = 1
        nameLbl.Text = contact.displayName or contact.username or "Unknown"
        nameLbl.TextColor3 = Color3.new(1, 1, 1)
        nameLbl.Font = hasUnread and Enum.Font.GothamBlack or Enum.Font.GothamBold
        nameLbl.TextSize = 12
        nameLbl.TextXAlignment = Enum.TextXAlignment.Left

        local previewLbl = Instance.new("TextLabel", row)
        previewLbl.Size = UDim2.new(1, -120, 0, 14)
        previewLbl.Position = UDim2.new(0, 58, 0, 30)
        previewLbl.BackgroundTransparency = 1
        local previewText = contact.lastMsg and contact.lastMsg.text or "Tap untuk chat"
        previewLbl.Text = #previewText > 30 and previewText:sub(1, 30) .. "..." or previewText
        previewLbl.TextColor3 = hasUnread and Color3.fromRGB(200, 200, 205) or Color3.fromRGB(80, 80, 86)
        previewLbl.Font = hasUnread and Enum.Font.GothamBold or Enum.Font.Gotham
        ...
        if hasUnread then
            ...
            badgeNum.Text = contact.unread > 9 and "9+" or tostring(contact.unread)
            badgeNum.TextColor3 = Color3.fromRGB(0, 0, 0)
            badgeNum.Font = Enum.Font.GothamBlack
            badgeNum.TextSize = 8
        end

        row.MouseButton1Click:Connect(function()
            openedConversation = contact.userId
            refreshCurr()
        end)
    end
end

-- ================= JALANKAN MESSAGE CHECKER =================
task.spawn(function()
    task.wait(5)
    checkMessageNotif()
end)
